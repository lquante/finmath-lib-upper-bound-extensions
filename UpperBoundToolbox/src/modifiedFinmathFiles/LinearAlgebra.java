...
/**
 * This class implements some methods from linear algebra (e.g. solution of a
 * linear equation, PCA).
 *
 * It is basically a functional wrapper using either the Apache commons math or
 * JBlas
 *
...
public class LinearAlgebra {
	...
	private static boolean isEigenvalueDecompositionViaSVD = Boolean.parseBoolean(
			System.getProperty("net.finmath.functions.LinearAlgebra.isEigenvalueDecompositionViaSVD", "false"));
	...
	static {
		...
		boolean isSolverUseApacheCommonsMath = Boolean
				.parseBoolean(System.getProperty("net.finmath.functions.LinearAlgebra.isUseApacheCommonsMath", "true"));
		...
		if (!isSolverUseApacheCommonsMath) {
			try {
				double[] x = org.jblas.Solve.solve(new org.jblas.DoubleMatrix(2, 2, 1.0, 1.0, 0.0, 1.0),
						new org.jblas.DoubleMatrix(2, 1, 1.0, 1.0)).data;
	 ...
	 * </ul>
	 * using a standard Tikhonov regularization, i.e., we solve in the least square
	 * sense A* x = b* where A* = (A^T, lambda I)^T and b* = (b^T , 0)^T.
	 *
	 * @param matrixA The matrix A (left hand side of the linear equation).
	 * @param b       The vector (right hand of the linear equation).
	 * @param lambda  The parameter lambda of the Tikhonov regularization. Lambda
	 *                effectively measures which small numbers are considered zero.
	...
	public static double[] solveLinearEquationTikonov(double[][] matrixA, double[] b, double lambda) {
		...
		/*
		 * The copy of the array is inefficient, but the use cases for this method are
		 * currently limited. And SVD is an alternative to this method.
		 */
		...
		DecompositionSolver solver = new QRDecomposition(new Array2DRowRealMatrix(matrixRegularized, false))
				.getSolver();
	 ...
	 * <li>x is an n - vector given as double[n],</li>
	 * </ul>
	 * using a Tikhonov regularization, i.e., we solve in the least square sense A*
	 * x = b* where A* = (A^T, lambda0 I, lambda1 S, lambda2 C)^T and b* = (b^T , 0
	 * , 0 , 0)^T.
	 *
	 * The matrix I is the identity matrix, effectively reducing the level of the
	 * solution vector. The matrix S is the first order central finite difference
	 * matrix with -lambda1 on the element [i][i-1] and +lambda1 on the element
	 * [i][i+1] The matrix C is the second order central finite difference matrix
	 * with -0.5 lambda2 on the element [i][i-1] and [i][i+1] and lambda2 on the
	 * element [i][i].
	 *
	 * @param matrixA The matrix A (left hand side of the linear equation).
	 * @param b       The vector (right hand of the linear equation).
	 * @param lambda0 The parameter lambda0 of the Tikhonov regularization. Reduces
	 *                the norm of the solution vector.
	 * @param lambda1 The parameter lambda1 of the Tikhonov regularization. Reduces
	 *                the slope of the solution vector.
	 * @param lambda2 The parameter lambda1 of the Tikhonov regularization. Reduces
	 *                the curvature of the solution vector.
	 * @return The solution x of the equation A* x = b*
	 */
	public static double[] solveLinearEquationTikonov(double[][] matrixA, double[] b, double lambda0, double lambda1,
			double lambda2) {
	...
	public static double[] solveLinearEquation(double[][] matrixA, double[] b) {
		...
		if (isSolverUseApacheCommonsMath) {
			...
			// Using SVD - very slow
			// solver = new SingularValueDecomposition(new
			// Array2DRowRealMatrix(A)).getSolver();
			...
			// For use of colt:
			// cern.colt.matrix.linalg.Algebra linearAlgebra = new
			// cern.colt.matrix.linalg.Algebra();
			// return linearAlgebra.solve(new DenseDoubleMatrix2D(A),
			// linearAlgebra.transpose(new DenseDoubleMatrix2D(new double[][] { b
			// }))).viewColumn(0).toArray();

			// For use of parallel colt:
			// return new
			// cern.colt.matrix.tdouble.algo.decomposition.DenseDoubleLUDecomposition(new
			// cern.colt.matrix.tdouble.impl.DenseDoubleMatrix2D(A)).solve(new
			// cern.colt.matrix.tdouble.impl.DenseDoubleMatrix1D(b)).toArray();
		}
	}
	...
	public static double[] solveLinearEquationSymmetric(double[][] matrix, double[] vector) {
		if (isSolverUseApacheCommonsMath) {
			...
			return org.jblas.Solve.solveSymmetric(new org.jblas.DoubleMatrix(matrix),
					new org.jblas.DoubleMatrix(vector)).data;
			/*
			 * To use the linear algebra package colt from cern.
			 * cern.colt.matrix.linalg.Algebra linearAlgebra = new
			 * cern.colt.matrix.linalg.Algebra(); double[] x = linearAlgebra.solve(new
			 * DenseDoubleMatrix2D(A), linearAlgebra.transpose(new DenseDoubleMatrix2D(new
			 * double[][] { b }))).viewColumn(0).toArray();
			 * 
			 * return x;
			 */
		}
	...
	/**
	 * Find a solution of the linear equation A x = b in the least square sense
	 * where
	 * <ul>
	 * <li>A is an n x m - matrix given as double[n][m]</li>
	 * <li>b is an m - vector given as double[m],</li>
	...
	public static double[] solveLinearEquationLeastSquare(double[][] matrix, double[] vector) {
		...
		DecompositionSolver solver = new SingularValueDecomposition(new Array2DRowRealMatrix(matrix, false))
				.getSolver();
	...
	/**
	 * Returns the matrix of the n Eigenvectors corresponding to the first n largest
	 * Eigenvalues of a correlation matrix. These Eigenvectors can also be
	 * interpreted as "principal components" (i.e., the method implements the PCA).
	 *
	 * @param correlationMatrix The given correlation matrix.
	 * @param numberOfFactors   The requested number of factors (eigenvectors).
	 * @return Matrix of n Eigenvectors (columns) (matrix is given as
	 *         double[n][numberOfFactors], where n is the number of rows of the
	 *         correlationMatrix.
	 */
	...
	/**
	 * Returns a correlation matrix which has rank &lt; n and for which the first n
	 * factors agree with the factors of correlationMatrix.
	 *
	 * @param correlationMatrix The given correlation matrix.
	 * @param numberOfFactors   The requested number of factors (Eigenvectors).
	...
	private static double[][] getFactorMatrixUsingCommonsMath(double[][] correlationMatrix, int numberOfFactors) {
		// case if dimensions of correlation matrix < number of factors
		if (correlationMatrix.length < numberOfFactors)
		...
		if (isEigenvalueDecompositionViaSVD) {
			SingularValueDecomposition svd = new SingularValueDecomposition(
					new Array2DRowRealMatrix(correlationMatrix));
		...
		for (int factor = 0; factor < numberOfFactors; factor++) {
			...
			double signChange = eigenVectorMatrix[0][eigenVectorIndex] > 0.0 ? 1.0 : -1.0; // Convention: Have first
																							// entry of eigenvector
																							// positive. This is to make
																							// results more consistent.
			double eigenVectorNormSquared = 0.0;
			for (int row = 0; row < eigenValues.length; row++) {
				eigenVectorNormSquared += eigenVectorMatrix[row][eigenVectorIndex]
						* eigenVectorMatrix[row][eigenVectorIndex];
			}
			eigenValue = Math.max(eigenValue, 0.0);
			for (int row = 0; row < eigenValues.length; row++) {
				factorMatrix[row][factor] = signChange * Math.sqrt(eigenValue / eigenVectorNormSquared)
						* eigenVectorMatrix[row][eigenVectorIndex];
			}
		}
	...
	public static double[][] factorReductionUsingCommonsMath(double[][] correlationMatrix, int numberOfFactors) {
		...
		for (int row = 0; row < correlationMatrix.length; row++) {
			...
			if (sumSquared != 0) {
				...
				// This is a rare case: The factor reduction of a completely decorrelated system
				// to 1 factor
				for (int factor = 0; factor < numberOfFactors; factor++) {
		...
		double[][] reducedCorrelationMatrix = (new Array2DRowRealMatrix(factorMatrix)
				.multiply(new Array2DRowRealMatrix(factorMatrix).transpose())).getData();
	 ...
	 *
	 * Note: The function currently requires jblas. If jblas is not availabe on your
	 * system, an exception will be thrown. A future version of this function may
	 ...
	 * @return pseudoInverse The pseudo-inverse matrix P, such that A*P*A = A and
	 *         P*A*P = P
	 */
	...
	public static double[][] diag(double[] vector) {
		...
		// Note: According to the Java Language spec, an array is initialized with the
